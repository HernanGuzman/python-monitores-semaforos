...
def agentePapelYFosforos():
    global papel, fosforos, cantiadPuestasAgente1, cantidadMaxima  
    while cantiadPuestasAgente1 > 0:
        if len(papel) < cantidadMaxima and len(fosforos) < cantidadMaxima:
...
def agentePapelYTabaco():
    global papel, tabaco, cantiadPuestasAgente2, cantidadMaxima
    while cantiadPuestasAgente2 > 0:
        if len(papel) < cantidadMaxima and len(tabaco) < cantidadMaxima:
            semaphoreAgente.acquire()
...
def agenteFosforosYTabaco():
    ...
    while cantiadPuestasAgente3 > 0:
        if len(fosforos) < cantidadMaxima and len(tabaco) < cantidadMaxima:
            semaphoreAgente.acquire()
